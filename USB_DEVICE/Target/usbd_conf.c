...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
        // 注意：FIFO大小以字为单位，所以用字节大小除以4
        // 例如，如果端点大小是64字节（0x40），则FIFO大小应该是16字（64 / 4）
        // 但是，对于CDC命令端点，我们使用了最小的FIFO大小16个字
        HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x80);
        HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x40);
        // 鼠标HID IN端点 - 端点号 1
        HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, HID_MOUSE_EPIN_SIZE / 4);

        // 键盘HID IN端点 - 端点号 2
        HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 2, HID_KEYBOARD_EPIN_SIZE / 4);

        // CDC数据IN端点 - 端点号 3
        // 由于CDC数据端点的大小为64字节，我们将FIFO大小设置为相同的值
        HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 3, CDC_DATA_FS_MAX_PACKET_SIZE / 4);

        // CDC命令IN端点 - 端点号 4
        // 尽管CDC命令端点的包大小为8字节，但STM32的USB硬件可能要求最小FIFO大小为16个字
        // 因此，我们至少需要分配16个字，即64字节
        HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 4, 16); // 我们这里直接使用了字数，而不是将字节大小除以4
